...
import styles from "./css/ServerList.Modal.module.scss"

// 새로운 서버 생성 모달창 컴포넌트
const Modal = ({ modalInfo, setStates }) => {
  const { setModalPopup, setList } = setStates;
  const modalPage = modalInfo.page;
  const serverId = modalInfo.serverId;
  ...
  const ModalMain = () => {
    if (modalPage === 'createServer') return (
    ...
    if (modalPage === 'customizeServer') return (
      <>
    ...
    if (modalPage === 'setServer') {
      const remove = true;
      const [avatarImgSrc, setAvatarImgSrc] = useState('https://blog.kakaocdn.net/dn/bWtlhb/btryNbfLRzq/yUC15p0SlJttaS2nB5WQ9k/img.png');
      ...
      const Content = () => {
        ...
        if (page === 1) {
          ...
          const updateServerProfile = async () => {
            if (!newData) return;
            ...
            try {
              ...
              const serverUrl = process.env.NEXT_PUBLIC_SERVER_PORT ?
                `${protocol}//${hostname}:${process.env.NEXT_PUBLIC_SERVER_PORT}` :
                "http://localhost:3040";
              const response = await axios.patch(serverUrl + '/api/server/avatar/' + serverId, formData, {
                headers: {
          ...
          return (
            <div className={styles.content}>
              <h2>서버 개요</h2>
              <div className={styles.setServerProfile}>
              ...
              <div className={`${styles.saveArea} ${styles.aniPopUp}`}>
                ...
                <div className={styles.buttonArea}>
                  ...
                  <div className={styles.buttonSave} onClick={updateServerProfile}>
                    변경사항 저장하기
                  </div>
                </div>
              </div>
      ...
      return (
        <div className={remove ? `${styles.modalSetServer} ${styles.animatePopDown}` : `${styles.modalSetServer} ${styles.animatePopUp}`}>
  ...
  return (
    <div className={styles.modalRoot}>
      <div className={styles.bgDark} onClick={() => setModalPopup(null)}></div>
      <ModalMain></ModalMain>
    </div>
  )
}
