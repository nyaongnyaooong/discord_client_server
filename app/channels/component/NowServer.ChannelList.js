...
const ChannelList = ({ coreData, serverId, serverMembers, channelId, channels, setChannels, voiceParticipants }) => {
  ...
  const Channel = (props) => {
    ...
    if (type === 'chat') {
      return (
        <Link href={props.linkUrl} className={props.selected ? styles.itemChannelSelected : styles.itemChannel }>
          ...
          </div>
          {
            participants ?
              Object.keys(participants).map((participant, idx) => {
                console.log(participants[participant].avatar)
                return (
                  <div key={idx} className={styles.list}>
                    <div className={styles.participant}>
                      <div className={styles.avatar}>
                        <img src={participants[participant].avatar || 'https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FsdZ9b%2FbtsyWf4jOsz%2FR03mU0dFgcSdG4KrRK9fck%2Fimg.png'} />
                      </div>
